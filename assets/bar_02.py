...
r1 = plot.vbar(source=source,x='x',top='y',bottom=0,width=1,color='#ff31a2',border_radius=14, line_color='black', hover_fill_color='#FCBF49', legend_label='Tair')

labels = LabelSet(x='x', y='y', text='y', level='glyph',text_align='center', y_offset=5, source=source, text_color = '#FCBF49', text_font_size="18pt", angle=0)
plot.add_layout(labels)

tltl = """<i>Month:</i> <b>@x</b> <br> <i>Tair:</i> <b>@y</b>"""; plot.add_tools(HoverTool(tooltips=hovfun(tltl),renderers = [r1])); 

br.apply_theme(plot,'dark'); show(plot, );fname = "output/bar_02"; br.save_plot(plot,fname)
